...
class TestLogWorkout:
    ...
    def test_appends_to_existing_day(self):
        ...
        payload = {"exercise_id": "pushups", "sets": [{"reps": 10}], "date": "2026-01-15", "user_id": "user1"}
...
class TestListWorkoutDates:
    def test_no_workouts_returns_empty(self):
        r = client.get("/workouts", params={"user_id": "user1", "date_from": "2026-01-01", "date_to": "2026-01-31"})
        assert r.status_code == 200
        assert r.json() == {"dates": []}
    ...
    def test_filters_to_date_range(self):
        ...
        r = client.get("/workouts", params={"user_id": "user1", "date_from": "2026-01-15", "date_to": "2026-01-31"})
        assert r.status_code == 200
    ...
    def test_invalid_user_id_rejected(self):
        r = client.get("/workouts", params={"user_id": "../evil", "date_from": "2026-01-01", "date_to": "2026-01-31"})
        assert r.status_code == 400

    def test_invalid_date_from_rejected(self):
        r = client.get("/workouts", params={"user_id": "user1", "date_from": "not-a-date", "date_to": "2026-01-31"})
        assert r.status_code == 400

    def test_invalid_date_to_rejected(self):
        r = client.get("/workouts", params={"user_id": "user1", "date_from": "2026-01-01", "date_to": "not-a-date"})
        assert r.status_code == 400
