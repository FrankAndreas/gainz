...
def initialize_data() -> None:
    ...
    if not users_file.exists():
        _write_json(users_file, {
            "users": [
                {"id": "user1", "name": "Andreas", "created": datetime.now().isoformat()},
                {"id": "user2", "name": "Family Member", "created": datetime.now().isoformat()},
            ]
        })

    if not exercises_file.exists():
        _write_json(exercises_file, {
            "bodyweight": [
                {"id": "pushups", "name": "Push-ups", "category": "bodyweight", "muscle_groups": ["chest", "triceps"]},
                {"id": "squats", "name": "Squats", "category": "bodyweight", "muscle_groups": ["legs", "glutes"]},
                {"id": "planks", "name": "Planks", "category": "bodyweight", "muscle_groups": ["core"]},
            ],
            "dumbbell": [
                {"id": "bicep_curls", "name": "Bicep Curls", "category": "dumbbell", "muscle_groups": ["biceps"]},
                {"id": "shoulder_press", "name": "Shoulder Press", "category": "dumbbell", "muscle_groups": ["shoulders"]},
            ],
        })
...
async def list_workout_dates(
    ...
    try:
        dates = sorted(
            ...
            if (stem := entry.stem) and _DATE_RE.fullmatch(stem) and date_from <= stem <= date_to
        )
    except OSError:
